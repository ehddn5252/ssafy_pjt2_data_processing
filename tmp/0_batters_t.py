...
sql = "SELECT DISTINCT player_uid from new_new_event_batters"
cursor.execute(sql)
res = cursor.fetchall()
...
for i in range(0, len(res)):
    ...
    r=res[i]
    player_uid = r[0]
    sql = "select distinct season from new_new_event_batters where player_uid = %s"
    cursor.execute(sql, player_uid)
    res2 = cursor.fetchall()
    for s in res2:
        season = s[0]
        teamRes = cursor.fetchall()
        for t in teamRes:
            team = t[0]
            try:
                ...
                sql = "SELECT *" \
                      "FROM new_new_event_batter_counts " \
                      "where player_uid = %s " \
                      "and season = %s " \
                      "and team = %s " \
                      "and opponent_hand = %s"
                cursor.execute(sql, (player_uid, season, team, "L"))
                ...
                sql = "SELECT *" \
                      ...
                      "and team = %s " \
                      "and opponent_hand = %s"
                cursor.execute(sql, (player_uid, season, team, "R"))
                ...
                sql = "insert into new_batters(player_uid, season, team_name, name, position, left_hit_num, " \
                      ...
                      "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s," \
                      "%s, %s, %s)"
                cursor.execute(sql, (player_uid, season, team, name, position, left_hit_num, right_hit_num, left_twob_hit_num, right_twob_hit_num, left_threeb_hit_num, right_threeb_hit_num, left_hr_num, right_hr_num, left_pa_num, right_pa_num, left_at_bat_num, right_at_bat_num, left_rbi, right_rbi, right_game_num, left_game_num, 0, 0, left_bb_num, right_bb_num, left_ao_num, right_ao_num, left_go_num, right_go_num, left_so_num, right_so_num, 0, 0, left_sh_num, right_sh_num, left_sf_num, right_sf_num, left_ibb_num, right_ibb_num, left_strike_num, right_strike_num, left_ball_num, right_ball_num))
                conn.commit()
