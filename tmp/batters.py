...
for r in res:
    ...
    for s in res2:
        try:
            ...
            cursor.execute(sql, (player_uid, season))
            ...
            cursor.execute(sql, (True, player_uid, season, "L"))
            left_at_bat_num = cursor.fetchall()[0][0]
            sql = "select count(*) " \
                  "from new_event_batters " \
                  "where at_bat= %s and player_uid = %s and season = %s and opponent_hand = %s"
            cursor.execute(sql, (True, player_uid, season, "R"))
            ...
            sql = "insert into batters(player_uid, season, name, position, left_hit_num, " \
                  ...
                  "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s," \
                  "%s, %s)"
            cursor.execute(sql, (player_uid, season, name, position, left_hit_num, right_hit_num, left_twob_hit_num, right_twob_hit_num, left_threeb_hit_num, right_threeb_hit_num, left_hr_num, right_hr_num, left_pa_num, right_pa_num, left_at_bat_num, right_at_bat_num, left_rbi, right_rbi, right_game_num, left_game_num, 0, 0, left_bb_num, right_bb_num, left_ao_num, right_ao_num, left_go_num, right_go_num, left_so_num, right_so_num, 0, 0, left_sh_num, right_sh_num, left_sf_num, right_sf_num, left_ibb_num, right_ibb_num, left_strike_num, right_strike_num, left_ball_num, right_ball_num))
            conn.commit()
        except:
            pass
# conn.commit()
