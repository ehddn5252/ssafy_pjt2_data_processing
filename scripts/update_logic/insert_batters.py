...
for r in res:
    ...
    for t in resTeam:
        ...
        for s in res2:
            season = s[0]
            sql = "select full_name, primary_position_abbreviation from baseball_players where uid = %s and team = %s"
            cursor.execute(sql, (player_uid, team))
            ...
            sql = f"insert into {BATTERS_TABLE} (player_uid, season, name, position, team_name left_hit_num, " \
                  ...
                  "%s, %s)"
            cursor.execute(sql, (
            player_uid, season, name, position, team, left_hit_num, right_hit_num, left_twob_hit_num,
            right_twob_hit_num, left_threeb_hit_num, right_threeb_hit_num, left_hr_num, right_hr_num, left_pa_num,
            right_pa_num, left_at_bat_num, right_at_bat_num, left_rbi, right_rbi, right_game_num, left_game_num, 0, 0,
            left_bb_num, right_bb_num, left_ao_num, right_ao_num, left_go_num, right_go_num, left_so_num, right_so_num,
            0, 0, left_sh_num, right_sh_num, left_sf_num, right_sf_num, left_ibb_num, right_ibb_num, left_strike_num,
