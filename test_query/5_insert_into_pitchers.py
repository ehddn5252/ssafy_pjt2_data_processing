...
def insert_into_pitchers(dml_instance, _player_uid, _season):
    ...
    sql = f"select event, is_hit, at_bat, pa, count,rbi  from new_event_pitchers where player_uid = {_player_uid} and season = '{_season}' and opponent_hand='L'"
    ...
    # 오른손 상대
    sql = f"select event, is_hit, at_bat, pa, count,rbi  from new_event_pitchers where player_uid = {_player_uid} and season = '{_season}' and opponent_hand='R'"
    new_events: Tuple = dml_instance.execute_fetch_sql(sql, [])
    try:
        for new_event in new_events:
